...
from espnet2.s2t_wadp.ctc import CTC
...
class ESPnetS2TWATModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 1. Encoder
        encoder_out, encoder_out_lens, l_qk, a_q_f = self.encode(speech, speech_lengths)
        intermediate_outs = None
        if isinstance(encoder_out, tuple):
            intermediate_outs = encoder_out[1]
            encoder_out = encoder_out[0]

        loss_att, acc_att, cer_att, wer_att, l_qk_att, loss_total = None, None, None, None, None, None
        ...
        if self.ctc_weight != 1.0:
            loss_att, acc_att, cer_att, wer_att, l_qk_att, loss_total = self._calc_att_loss(
                encoder_out,
                encoder_out_lens,
                l_qk,
                text,
        ...
        if self.ctc_weight == 0.0:
            ...
            loss = self.ctc_weight * loss_ctc + (1 - self.ctc_weight) * loss_total
        ...
        stats["cer"] = cer_att
        stats["wer"] = wer_att
        stats["loss_rank"] = l_qk
        stats["total_loss"] = loss_total
        
        # rank-related stats
        num_layer = len(a_q_f)

        for l_index in range(num_layer):
            stats["Layer_"+str(l_index+1)] = torch.count_nonzero(a_q_f[l_index])
    ...
    def encode(
        ...
        if self.encoder.interctc_use_conditioning:
            encoder_out, encoder_out_lens, l_qk, a_q_f, _ = self.encoder(
                feats, feats_lengths, ctc=self.ctc
            )
        else:
            encoder_out, encoder_out_lens, l_qk, a_q_f, _ = self.encoder(feats, feats_lengths)
        intermediate_outs = None
        if isinstance(encoder_out, tuple):
            intermediate_outs = encoder_out[1]
            encoder_out = encoder_out[0]
        ...
        if intermediate_outs is not None:
            return (encoder_out, intermediate_outs), encoder_out_lens, l_qk, a_q_f

        return encoder_out, encoder_out_lens, l_qk, a_q_f
    ...
    def _calc_att_loss(
        ...
        return loss_att, acc_att, cer_att, wer_att, l_qk, loss_total
